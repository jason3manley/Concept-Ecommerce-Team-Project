...
<%	float totpri=0;
	...
	ResultSet  rs1;
	rs1= st.executeQuery("select ordernum from login where userid=(select user_id from user where user_name= '"+user+"' )");
	while(rs1.next()){ ordNum=rs1.getInt("ordernum"); 	}
	
	ResultSet rs2;
	rs2 = st.executeQuery("select prod_id from orders where order_id='"+ordNum+"' and IsPay = 0");
